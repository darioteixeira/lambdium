...
CREATE FUNCTION get_login (text, text)
...
BEGIN
	...
	IF FOUND
	THEN
		IF _target_password = _actual_user.user_password_hash
		THEN
...
/*
 * Adds a new user.  Note that this is the default version of this
 * function, one that does not really salt and encrypt the password.
 */

CREATE FUNCTION add_user (text, text, text, int4)
RETURNS void
LANGUAGE plpgsql VOLATILE AS
$$
DECLARE
	_nick			ALIAS FOR $1;
	_fullname		ALIAS FOR $2;
	_password		ALIAS FOR $3;
	_timezone_id		ALIAS FOR $4;
	_password_salt		text;
	_password_hash		text;

BEGIN
	_password_salt := 'dummy';
...
CREATE FUNCTION edit_user_credentials (int4, text, text)
RETURNS void
LANGUAGE plpgsql VOLATILE AS
$$
DECLARE
	_user_id		ALIAS FOR $1;
	_user_password		ALIAS FOR $2;
...
BEGIN
	...
	IF FOUND
	THEN
		IF _old_password = _actual_user.user_password_hash
		THEN
			UPDATE	users
				SET user_password_hash = _user_password
...
BEGIN
	UPDATE	users
		SET (user_full_name, user_timezone_id) = (_user_fullname, _user_timezone_id)
		WHERE user_id = _user_id;
END
$$;
